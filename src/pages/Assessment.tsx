...
const assessmentData: Record<string, CompanyAssessmentData> = {
  meridian: {
    techStack: [
      { name: 'RailSentry / LIDAR', category: 'Track Inspection', current: 7, target: 9 },
      { name: 'HCSS Telematics + Equipment360', category: 'Fleet & Equipment', current: 7, target: 9 },
      { name: 'eCMS (Computer Guidance)', category: 'ERP', current: 3, target: 7 },
      { name: 'QMirror / AS400', category: 'Legacy Financials', current: 1, target: 6 },
      { name: 'Primavera P6', category: 'Project Controls', current: 5, target: 8 },
      { name: 'Procore', category: 'Construction Mgmt', current: 8, target: 9 },
      { name: 'HCSS Safety', category: 'Safety', current: 7, target: 9 },
  ...
  hcc: {
    techStack: [
      { name: 'HCSS Heavy Job', category: 'Construction Mgmt', current: 6, target: 9 },
      { name: 'Primavera P6', category: 'Project Controls', current: 5, target: 8 },
      { name: 'eCMS (Computer Guidance)', category: 'ERP', current: 3, target: 7 },
      { name: 'HCSS Telematics', category: 'Fleet Tracking', current: 7, target: 9 },
      { name: 'Trimble Earthworks', category: 'Machine Control', current: 6, target: 8 },
      { name: 'Procore', category: 'Project Mgmt', current: 8, target: 9 },
    ],
    licenses: [
      { vendor: 'HCSS Heavy Job', total: 180, active: 142, waste: 76000, action: 'Remove inactive field licenses from completed projects' },
      { vendor: 'Primavera P6', total: 85, active: 62, waste: 92000, action: 'Downgrade 15 seats to viewer-only' },
      { vendor: 'HCSS Telematics', total: 220, active: 200, waste: 24000, action: 'Decommission 20 retired vehicle trackers' },
      { vendor: 'AutoCAD', total: 45, active: 28, waste: 68000, action: 'Consolidate with HTI shared license pool' },
    ],
  },
  hrsi: {
    techStack: [
      { name: 'HCSS Telematics', category: 'Equipment Tracking', current: 7, target: 9 },
      { name: 'eCMS (Computer Guidance)', category: 'ERP', current: 3, target: 7 },
      { name: 'MCP (Internal Payroll)', category: 'Workforce Mgmt', current: 3, target: 7 },
      { name: 'HCSS Heavy Job', category: 'Field Ops', current: 6, target: 9 },
      { name: 'Equipment360', category: 'Asset Mgmt', current: 7, target: 9 },
      { name: 'Business Objects', category: 'Analytics', current: 3, target: 7 },
    ],
    licenses: [
      { vendor: 'eCMS', total: 65, active: 48, waste: 34000, action: 'Consolidate with HCC eCMS instance' },
      { vendor: 'MCP Payroll', total: 140, active: 129, waste: 11000, action: 'Remove inactive seasonal workers' },
      { vendor: 'Equipment360', total: 80, active: 68, waste: 18000, action: 'Migrate 12 unused mobile licenses' },
    ],
  },
  hsi: {
    techStack: [
  ...
  htsi: {
    ...
    licenses: [
      ...
      { vendor: 'MCP Payroll', total: 200, active: 180, waste: 20000, action: 'Consolidate with HRSI MCP instance' },
  ...
  he: {
    ...
    licenses: [
      ...
      { vendor: 'eCMS', total: 30, active: 22, waste: 16000, action: 'Consolidate into parent eCMS tenant' },
    ],
  },
  gg: {
    techStack: [
...
const extendedData: Record<string, ExtendedAssessmentData> = {
  meridian: {
    integrations: [
      { from: 'RailSentry LIDAR', to: 'eCMS', status: 'siloed', dataFlow: 'No integration \u2014 manual GIS overlay by HSI engineers' },
      { from: 'HCSS Telematics', to: 'eCMS', status: 'partial', dataFlow: 'Weekly fleet position batch upload via flat-file export' },
      { from: 'P6 Schedules', to: 'eCMS', status: 'partial', dataFlow: 'Bi-directional sync via middleware, some manual reconciliation' },
      { from: 'MCP Payroll', to: 'eCMS', status: 'siloed', dataFlow: 'Crew hours rekeyed manually into eCMS payroll module' },
      { from: 'QMirror / AS400', to: 'eCMS', status: 'siloed', dataFlow: 'Batch lag \u2014 AS/400 flat-file exports, 12-24hr data delay' },
      { from: 'Business Objects', to: 'eCMS', status: 'connected', dataFlow: 'ODBC direct connection' },
    ],
    remediation: [
      { priority: 1, action: 'Deploy track intelligence data lake to unify RailSentry + Video Track Chart + HSI Ultrasonic data', category: 'integration', effort: 'High', timeline: '3-4 months', impact: '$1.38M defect detection savings' },
      { priority: 2, action: 'Unblock QMirror/AS400 bottleneck \u2014 real-time API bridge to eCMS', category: 'integration', effort: 'High', timeline: '4-5 months', impact: 'Eliminate 12-24hr data lag across all divisions' },
      { priority: 3, action: 'Consolidate 4 eCMS division instances to single tenant', category: 'optimization', effort: 'High', timeline: '4-5 months', impact: '$480K annual license savings' },
      { priority: 4, action: 'Connect HCSS Telematics to Equipment360 for real-time fleet visibility', category: 'integration', effort: 'Medium', timeline: '6-8 weeks', impact: '$320K annual savings' },
  ...
  hcc: {
    integrations: [
      { from: 'HCSS Heavy Job', to: 'eCMS', status: 'partial', dataFlow: 'Daily batch upload of cost data' },
      { from: 'Primavera P6', to: 'eCMS', status: 'connected', dataFlow: 'Bi-directional sync via middleware' },
      { from: 'HCSS Telematics', to: 'Equipment360', status: 'connected', dataFlow: 'Real-time fleet telemetry' },
      { from: 'Trimble Earthworks', to: 'HCSS Heavy Job', status: 'siloed', dataFlow: 'Manual grade data export' },
    ],
    remediation: [
      { priority: 1, action: 'Integrate Trimble machine control data into Heavy Job for automated grade reporting', category: 'integration', effort: 'Medium', timeline: '6-8 weeks', impact: 'Eliminate 3-hr daily manual reconciliation' },
  ...
  hrsi: {
    integrations: [
      { from: 'HCSS Telematics', to: 'eCMS', status: 'partial', dataFlow: 'Weekly position batch upload via flat-file' },
      { from: 'MCP Payroll', to: 'HCSS Heavy Job', status: 'connected', dataFlow: 'Shared API data source' },
      { from: 'Equipment360', to: 'eCMS', status: 'siloed', dataFlow: 'Manual work order entry' },
    ],
    remediation: [
      { priority: 1, action: 'Integrate Equipment360 with eCMS for automated work order generation', category: 'integration', effort: 'Medium', timeline: '8 weeks', impact: 'Reduce work order entry time by 4 hrs/day' },
      { priority: 2, action: 'Deploy real-time HCSS Telematics feed to eCMS', category: 'integration', effort: 'Medium', timeline: '6 weeks', impact: '$89K maintenance cost avoidance' },
    ],
  },
  hsi: {
    integrations: [
  ...
  htsi: {
    integrations: [
      ...
      { from: 'MCP Payroll', to: 'Trapeze OPS', status: 'siloed', dataFlow: 'Manual crew schedule sync' },
    ],
    remediation: [
      { priority: 1, action: 'Integrate MCP crew data with Trapeze for automated rostering', category: 'integration', effort: 'Medium', timeline: '8 weeks', impact: 'Reduce scheduling errors 40%' },
  ...
  he: {
    integrations: [
      ...
      { from: 'Permit Tracking', to: 'eCMS', status: 'siloed', dataFlow: 'Manual cost allocation' },
      { from: 'Weather API', to: 'SCADA', status: 'connected', dataFlow: 'Hourly forecast integration' },
    ],
    remediation: [
      { priority: 1, action: 'Automate permit cost tracking in eCMS', category: 'integration', effort: 'Low', timeline: '4 weeks', impact: 'Eliminate 6 hrs/week manual entry' },
      { priority: 2, action: 'Consolidate eCMS into parent tenant', category: 'optimization', effort: 'Medium', timeline: '8 weeks', impact: '$16K annual savings' },
    ],
  },
  gg: {
    integrations: [
